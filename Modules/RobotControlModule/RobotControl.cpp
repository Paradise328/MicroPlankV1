...
void RobotControl::initiAllData()
{
    ...
    // m_ruckigInputState_R.max_velocity = {800000.0, 800000.0, 800000.0};//500000.0
    // m_ruckigInputState_R.max_acceleration = {30000.0, 30000.0, 30000.0};//15000.0
    // m_ruckigInputState_R.max_jerk = {8500.0, 8500.0, 8500.0};//8000.0

    // m_ruckigInputState_L.max_velocity = {800000.0, 800000.0, 800000.0};
    // m_ruckigInputState_L.max_acceleration = {30000.0, 30000.0, 30000.0};
    // m_ruckigInputState_L.max_jerk = {8500.0, 8500.0, 8500.0};
...
void RobotControl::teleoperation()
{
    ...
    auto motorStatusWord_L = m_motorStatusWordCur_L.load();
    auto motorTrq_L = m_motorTrq_L.load();
    auto motorCur_L = m_motorCur_L.load();
    auto motorFollowingPosErr_L = m_motorFollowingPosErr_L.load();
    ...
    if(enableTagCur_L == enableAction||enableTagCur_L == keepEnabling)
    {
        ...
        // LOG(INFO)<<std::dec<<"targetEncoder_L[9]: "<<targetEncoder_L[9]<<" targetEncoder_L[8]: "<<targetEncoder_L[8]<<" targetEncoder_L[7]: "<<targetEncoder_L[7]<<" targetEncoder_L[6]: "<<targetEncoder_L[6];
...
/*jacobi*/
         ...
         // targetVelocity_L_new[0] = m_jointAngle0_velocity_L;
         // targetVelocity_L_new[1] = m_jointAngle1_velocity_L;
         // targetVelocity_L_new[2] = m_jointAngle2_velocity_L;
         // targetVelocity_L_new[3] = m_jointAngle3_velocity_L;
...
{   
    ...
    auto openAngle_L_new = (openAngle_L < 0) ? 0.010 * pow(openAngle_L, 3) : pow(openAngle_L, 3)/600;
    ...
    // outfile1 << (endEffectorInit_X_L + endEffectorDelta_X_L) << " " << (endEffectorInit_Y_L + endEffectorDelta_Y_L) << " " << (endEffectorInit_Z_L + endEffectorDelta_Z_L) << " "
    //          << m_ruckigOutputState_L.new_position[0] << " " << m_ruckigOutputState_L.new_position[1] << " " << m_ruckigOutputState_L.new_position[2] << "\n";
...
/*自适应低通滤波*/
    ...
    // auto endEffectorAcutalPosition_L = calculateEndEffectorPosition(motorPositionCur_L, 'l');
    // m_ruckigInputState_L.current_position = {endEffectorAcutalPosition_L[0], endEffectorAcutalPosition_L[1], endEffectorAcutalPosition_L[2]};
...

{
    ...
    auto openAngle_R_new = (openAngle_R < 0) ? 0.010 * pow(openAngle_R, 3) : pow(openAngle_R, 3)/600;
   ...
   m_compensation_CurR.z() = compensation_sum.z();
    ...
    // auto endEffectorAcutalPosition_R = calculateEndEffectorPosition(motorPositionCur_R, 'r');
    // m_ruckigInputState_L.current_position = {endEffectorAcutalPosition_R[0], endEffectorAcutalPosition_R[1], endEffectorAcutalPosition_R[2]};
    ...
    /*4轴器械*/
    if(m_endeffectorConfiguration == EndeffectorConfiguration::fourMaxons){
        // controlValueTmp_R[4] = - openAngle_R_new;
        // controlValueTmp_R[5] = + openAngle_R_new;
        /*四轴*/
...
// LOG(INFO)<<"endEffectorDelta_X_L: "<<endEffectorDelta_X_L;
    ...
    std::ofstream outfile("viperjacobi.txt",std::ios::app);
    outfile<<X_L<<" "<<Y_L<<" "<<Z_L<< "\n";
     if(test_index == 6){
        outfile<<" =========================================================================== "<<"\n";
     }
                                                                       ...
                                                                       const char& side)const
{
    ...
    if(side == 'r')
    {
        ...
        if(m_endeffectorConfiguration == EndeffectorConfiguration::sixMaxons)
        {
            for(int i = 4; i < 10; i++)
            {
                targetEncoder[i] = static_cast<int>(controlValue_Cur[i] * m_kForcepPosition_R[13 - i]);
            }
        }
        if(m_endeffectorConfiguration == EndeffectorConfiguration::fourMaxons)
        {
            ...
            // for(int i = 4; i < 8; i++)
            // {
            //     targetEncoder[i] = static_cast<int>(controlValue_Cur[i] * m_encoderPerDegree_R[i - 4]) * 2.75;
            // }
             targetEncoder[4] =0;
             targetEncoder[5] =0;
    ...
    else if(side == 'l')
    {
        ...
        if(m_endeffectorConfiguration == EndeffectorConfiguration::sixMaxons)
        {
            for(int i = 4; i < 10; i++)
            {
                targetEncoder[i] = static_cast<int>(controlValue_Cur[i] * m_kForcepPosition_L[13 - i]);
            }
        }
        if(m_endeffectorConfiguration == EndeffectorConfiguration::fourMaxons)
        {
            // for(int i = 4; i < 8; i++)
            // {
            //     targetEncoder[i] = static_cast<int>(controlValue_Cur[i] * m_encoderPerDegree_L[i - 4]) * 2.75;
            // }
            targetEncoder[4] =0;
            targetEncoder[5] =0;
                                                                      ...
                                                                      const char& side)const
{
    ...
    if(side == 'r')
    {
        ...
        double rounded = round(targetEncoder[0] * 10000.0) / 10000.0;
        targetEncoder[0] = rounded;

        targetEncoder[0] = (motorPosition_Init[0] + controlValue_Cur[0] * 4000);
        for(int i = 1; i < 4; i++)
        {
            targetEncoder[i] = (motorPosition_Init[i] + m_SpeedDirection_R[i] * controlValue_Cur[i] / 360 * JointEncoderPerRevolution);
        }
        if(m_endeffectorConfiguration == EndeffectorConfiguration::sixMaxons)
        {
            for(int i = 4; i < 10; i++)
    ...
    else if(side == 'l')
    {
        ...
        for(int i = 1; i < 4; i++)
        {
            targetEncoder[i] = (motorPosition_Init[i] + m_SpeedDirection_L[i] * controlValue_Cur[i] / 360 * JointEncoderPerRevolution);
                                                                        ...
                                                                        const char& side)
{
    ...
    // LOG(INFO) << std::dec<<targetVel[0] << " "<< targetVel[1] << " "<< targetVel[2] << " "<< targetVel[3];
    }
...
void RobotControl::receiveDomainController(){
    // LOG(INFO) << "domainController info: "
              // << m_domainController_L->m_domainControllerData.TestByte;
    // LOG(INFO) << "L domain controller info: "
    //           << m_domainController_L->getForce()[0]<<" "
    //           << m_domainController_L->getForce()[1]<<" "
    //           << m_domainController_L->getForce()[2];
    // LOG(INFO) << "R domain controller info: "
    //           << m_domainController_R->getForce()[0]<<" "
    //           << m_domainController_R->getForce()[1]<<" "
    //           << m_domainController_R->getForce()[2];
}
...
void RobotControl::receiveMotorData()
{
    ...
    /*力*/
    auto force_l = m_domainController->getForce(1);
    auto force_r = m_domainController->getForce(2);
    // std::cout << "force_l: " << force_l[0] << " " << force_l[1]<< " " << force_l[2] << std::endl;
    // std::cout << "force_r: " << force_r[0] << " " << force_r[1]<< " " << force_r[2] << std::endl;
    ...
    // if((domainDigtalPrev_L == 0 && domainDigitalCur_L == 4) ||(domainDigtalPrev_R == 0 && domainDigitalCur_R == 4) &&)
    // {
    //     goToCollaboration_EndJoint();
    // }
    // if((domainDigtalPrev_L == 4 && domainDigitalCur_L == 0) ||(domainDigtalPrev_R == 4 && domainDigitalCur_R == 0))
    // {
    //     if(m_prevRobotControlMode.load() == RobotControlMode::TeleOperation)
    //     {
    //         goToTeleOperation();
    //     }
    //     if(m_prevRobotControlMode.load() == RobotControlMode::Hold)
    //     {
    //         goToHold();
    //     }
    // }
...
/*new*/
    ...
    // motorFollowingPosErr_L[4] =  m_motorDriver->getFollowingPosErr(MotorType::MAXON, 0, arm_1);
    // motorFollowingPosErr_L[5] =  m_motorDriver->getFollowingPosErr(MotorType::MAXON, 1, arm_1);
    // motorFollowingPosErr_L[6] =  m_motorDriver->getFollowingPosErr(MotorType::MAXON, 2, arm_1);
    // motorFollowingPosErr_L[7] =  m_motorDriver->getFollowingPosErr(MotorType::MAXON, 3, arm_1);
    // if(m_endeffectorConfiguration == EndeffectorConfiguration::sixMaxons)
    // {
...
{
    if(static_cast<int>(domainDigitalCur_L) == 4)/*按下按钮*/
    {
        m_endJointDragBtnCounter_L++;
    }else if(static_cast<int>(domainDigitalCur_L) != 4)
    {
        m_endJointDragBtnCounter_L = 0;
        m_dragButtonPressCur_L = 0;
    }

    if(static_cast<int>(domainDigitalCur_R) == 4)
    {
        m_endJointDragBtnCounter_R++;
    }else if(static_cast<int>(domainDigitalCur_R) != 4)
    {
...
// void RobotControl::changeOperationMode(const HandlePose& masterHandlePose_Cur)
// {
//     m_curRobotControlMode.load();

//     if(masterHandlePose_Cur.enablePedal == pedalDisable && (m_endJointDragBtnCounter_L >= 99 || m_endJointDragBtnCounter_R >= 99)){
...
//         {
//             if((m_enableTagPrev_R == disableAction || m_enableTagPrev_R == keepDisabling)
//                 && ((masterHandlePose_Cur.enablePedal == pedalEnable)||(masterHandlePose_Cur.enablePedal == pedalDisable && m_dragButtonPress_R == 1))
...
//                      ((masterHandlePose_Cur.enablePedal == pedalEnable && isPoseMatch(masterHandlePose_Cur,'r') == false)||(masterHandlePose_Cur.enablePedal == pedalDisable && m_dragButtonPress_R == 0)))
//             {
//                 enableFlag = keepDisabling;
//             }
//             else if ((m_enableTagPrev_R == enableAction || m_enableTagPrev_R == keepEnabling) &&
//                        ((masterHandlePose_Cur.enablePedal == pedalDisable && m_dragButtonPress_R == 0) || (masterHandlePose_Cur.enablePedal == pedalDisable && m_dragButtonPress_R == 1)))
//             {
//                 // m_handlePoseLastLoop_R = masterHandlePose_Cur;
//                 enableFlag = disableAction;
//                 m_motorTargetEncoderLast_R = motorTargetEncoder_R;
...
//             }
//             else if ((m_enableTagPrev_R == enableAction || m_enableTagPrev_R == keepEnabling) &&
//                        ((m_dragButtonPress_R == 0 && masterHandlePose_Cur.enablePedal == pedalEnable) || (m_dragButtonPress_R == 1 && masterHandlePose_Cur.enablePedal == pedalDisable)))
//             {
//                 enableFlag = keepEnabling;
//             }
//         }
...
bool RobotControl::isPoseRight(const HandlePose& masterHandlePose_Cur, const char& side) const//yu 且要满足flag_openangle等于true时（false为status=3时openangle太小）
{
    ...
    // LOG(INFO)<<" endEffectorTarget_Y_L: "<<endEffectorTarget_Y_L<<" endEffectorTarget_Z_L: "<<endEffectorTarget_Z_L;
    // LOG(INFO)<<" endEffectorTarget_Y_R: "<<endEffectorTarget_Y_R<<" endEffectorTarget_Z_R: "<<endEffectorTarget_Z_R;
    // LOG(INFO)<<"masterHandlePose_Cur.handlePoseR_Y: "<<masterHandlePose_Cur.handlePoseR_Y<<" masterHandlePose_Cur.handlePoseL_X: "<<masterHandlePose_Cur.handlePoseL_X;

    // if(m_oprationMode == 1){

     if(side =='l'){

        if (handlePoseCur.handlePoseL_Roll * 180 / M_PI < 135 && handlePoseCur.handlePoseL_Roll * 180 / M_PI > -135){//67
        if (handlePoseCur.handlePoseL_Elevation * 180 / M_PI < 75 && handlePoseCur.handlePoseL_Elevation * 180 / M_PI > -75){//67

             if (handlePoseCur.handlePoseL_Arzimuth * 180 / M_PI < 110 && handlePoseCur.handlePoseL_Arzimuth * 180 / M_PI > -110){//87

                 if((masterHandlePose_Cur.handlePoseL_X > -25) && (masterHandlePose_Cur.handlePoseL_X < 80)){

                     if((masterHandlePose_Cur.handlePoseL_Y > -50) && (masterHandlePose_Cur.handlePoseL_Y < 50)){

                         if((masterHandlePose_Cur.handlePoseL_Z > -80) && (masterHandlePose_Cur.handlePoseL_Z < -15)){

                             // if(((endEffectorTarget_Y_L < -245) && (handlePoseCur.handlePoseL_Y - m_handlePosePrev.handlePoseL_Y) < 0) || (endEffectorTarget_Y_L > -245)){

                             //     if(((endEffectorTarget_Y_L > - 175) && (handlePoseCur.handlePoseL_Y - m_handlePosePrev.handlePoseL_Y) > 0) || (endEffectorTarget_Y_L < -175)){

                             //         if(((endEffectorTarget_Z_L < -555) && (handlePoseCur.handlePoseL_Z - m_handlePosePrev.handlePoseL_Z) < 0) || (endEffectorTarget_Z_L > -555)){

                             //             if(((endEffectorTarget_Z_L > -485) && (handlePoseCur.handlePoseL_Z - m_handlePosePrev.handlePoseL_Z) > 0) || (endEffectorTarget_Z_L < -485)){
     ...
     if(side =='r'){
        ...
        if (handlePoseCur.handlePoseR_Elevation * 180 / M_PI < 75  && handlePoseCur.handlePoseR_Elevation * 180 / M_PI > -75){
             ...
             if (handlePoseCur.handlePoseR_Arzimuth * 180 / M_PI < 110 && handlePoseCur.handlePoseR_Arzimuth * 180 / M_PI > -110){

                 if((masterHandlePose_Cur.handlePoseR_X > -25) && (masterHandlePose_Cur.handlePoseR_X < 80)){

                     if((masterHandlePose_Cur.handlePoseR_Y > -50) && (masterHandlePose_Cur.handlePoseR_Y < 50)){

                         if((masterHandlePose_Cur.handlePoseR_Z > -80) && (masterHandlePose_Cur.handlePoseR_Z < -15)){

                             // if(((endEffectorTarget_Y_R < 175) && (handlePoseCur.handlePoseR_Y - m_handlePosePrev.handlePoseR_Y) < 0) || (endEffectorTarget_Y_R > 175)){

                             //     if(((endEffectorTarget_Y_R > 245) && (handlePoseCur.handlePoseR_Y - m_handlePosePrev.handlePoseR_Y) > 0) || (endEffectorTarget_Y_R < 245)){

                             //         if(((endEffectorTarget_Z_R < 485) && (handlePoseCur.handlePoseR_Z - m_handlePosePrev.handlePoseR_Z) < 0) || (endEffectorTarget_Z_R > 485)){

                             //             if(((endEffectorTarget_Z_R > 555) && (handlePoseCur.handlePoseR_Z - m_handlePosePrev.handlePoseR_Z) > 0) || (endEffectorTarget_Z_R < 555)){
...
void RobotControl::zeroErrGoHome(const char& side)
{
    if(side == 'r')
    {
        ...
        if(m_armAnglePerSide == 30){
            ...
            targetPosition = {JointEncoderPerRevolution / 2 + static_cast<int>(54.4 * EncoderPerGrade),
                              JointEncoderPerRevolution / 2 + static_cast<int>(98.4 * EncoderPerGrade),
                              JointEncoderPerRevolution / 2 + static_cast<int>(14.2 * EncoderPerGrade)};
        }
        if(m_armAnglePerSide == 15){
            targetPosition = {JointEncoderPerRevolution / 2 + static_cast<int>(21.7338 * EncoderPerGrade),
                              JointEncoderPerRevolution / 2 + static_cast<int>(98.0397 * EncoderPerGrade),
                              JointEncoderPerRevolution / 2 + static_cast<int>(61.3059 * EncoderPerGrade)};
            // targetPosition = {JointEncoderPerRevolution / 2 + static_cast<int>(22 * EncoderPerGrade),
            //                   JointEncoderPerRevolution / 2 + static_cast<int>(97 * EncoderPerGrade),
            //                   JointEncoderPerRevolution / 2 + static_cast<int>(60 * EncoderPerGrade)};
        }
    ...
    else if(side == 'l')
    {
        ...
        if(m_armAnglePerSide == 30){
            // targetPosition = {JointEncoderPerRevolution / 2 - static_cast<int>(45.2336 * EncoderPerGrade),
            //                   JointEncoderPerRevolution / 2 - static_cast<int>(97.0599 * EncoderPerGrade),
            //                   JointEncoderPerRevolution / 2 - static_cast<int>(21.8223* EncoderPerGrade)};//21.8223

            targetPosition = {JointEncoderPerRevolution / 2 - static_cast<int>(54.4 * EncoderPerGrade),
                              JointEncoderPerRevolution / 2 - static_cast<int>(98.2 * EncoderPerGrade),
                              JointEncoderPerRevolution / 2 - static_cast<int>(14.2 * EncoderPerGrade)};
        }
        if(m_armAnglePerSide == 15){
            targetPosition = {JointEncoderPerRevolution / 2 - static_cast<int>(21.7338 * EncoderPerGrade),
...
void RobotControl::setNewSpeed(const HandlePose& handlePosePrev, const HandlePose& handlePoseCur)
{
    ...
    if(handlePoseCur.stepPedal == pedalDeceleration && handlePosePrev.stepPedal == pedalNoAction)
    {
        if(m_speedPedalIndex_Cur > 0)
        {
            ...
            arglist.append("speedCur:"+QString::number(m_speedPedalIndex_Cur));
            SendInnerMsg(Module_Inner_E::Uiinterface,static_cast<int>(UIAction_E::RecvMasterData),arglist);
            if(m_speedPedalIndex_Cur == pedalSwitchThree)
            {
