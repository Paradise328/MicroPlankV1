...
int viper_ui::detect_input(){
  ...
  if (!is_continuous)
  {
    ...
    if (rv!=0)
    {
      std::cout<<"Error starting continuous ("<<rv<<")\n";
    }
  }
  ...
  while (is_continuous)
  {
    ...
    {  // did we read it all?
      ...
      if (crc==*(uint32_t*)(resp_pkg+br-4))
      {
        ...
        std::array<std::array<double,viperDataNumPerSensor>,2> sensorData;

        for (i = 0; i < num_sens; i++)
        {
          for(int j  = 0; j < 3; j++)
          {
            sensorData[0][j] = pfd->pno.pos[j];
            sensorData[1][j] = (pfd+1)->pno.pos[j];
            sensorData[0][j+3] = pfd->pno.ori[j];
            sensorData[1][j+3] = (pfd+1)->pno.ori[j];
          }
        }
        m_sensorData.store(sensorData);
      }
    }
  }
...
void viper_ui::stream_cont_accelerationMode(viper_usb* pvpr){

  const  uint32_t HDR_END_LOC=24;
  uint32_t crc,br,num_sens,i;


  uint32_t resp_size=sizeof(uint32_t)*11+sizeof(SENFRAMEDATA_A)*16+CRC_SIZE; // max no. of sensors
  uint8_t* resp_pkg=new uint8_t[resp_size];
  ...
  while (is_continuous)
  {
    ...
    {  // did we read it all?
      ...
      {
        num_sens=*(uint32_t*)(resp_pkg+20);
        pfd=(SENFRAMEDATA_A*)(resp_pkg+HDR_END_LOC);
        std::array<std::array<double,viperDataNumPerSensor>,2> sensorData;

          for(int j  = 0; j < 3; j++)
          {
            sensorData[0][j] = pfd->pno.pos[j];//0,1,2位位置
            sensorData[1][j] = (pfd+1)->pno.pos[j];
          }
          for(int j  = 0; j < 4; j++)
          {
            sensorData[0][j+3] = Quaternion_Fract2Float(pfd->pno.ori[j]);//3,4,5,6位为四元数
            sensorData[0][j+7] = Acceleration_Fract2Float((pfd)->pno.acc[j]);//7,8,9为加速度
            sensorData[1][j+3] = Quaternion_Fract2Float((pfd+1)->pno.ori[j]);
            sensorData[1][j+7] = Acceleration_Fract2Float((pfd+1)->pno.acc[j]);
          }
            sensorData[0][11] = pfd->SFinfo.bfDistortion;
            sensorData[1][11] = (pfd+1)->SFinfo.bfDistortion;
...
uint32_t viper_ui::start_continuous_acclerationMode(viper_usb* pvpr){
  ...
  // start collection thread
  is_continuous=1;
  cont_thread=thread(&viper_ui::stream_cont_accelerationMode,this,pvpr);

  return 0;
}

void viper_ui::print_pno_record_accelerationMode(SENFRAMEDATA_A* pfd){
...
uint32_t viper_ui::print_single_accelerationMode(viper_usb* pvpr,uint32_t reset_cont){
  ...
  pfd=(SENFRAMEDATA_A*)(resp_pkg+44);
  for (i=0;i<num_sens;i++)
    print_pno_record_accelerationMode(pfd+i);
  cout<<"\n";
...
{
  //Check if viper is connected well
  int try_ConnectUSBCount = 0;
  do
  {
    if(try_ConnectUSBCount != 0)
    {
      std::cout<<"\n\nError connecting to Viper over USB\n\n";
    }
      this_thread::sleep_for(std::chrono::milliseconds(200));
      try_ConnectUSBCount ++;
      std::cout << "try_ConnectUSBCount: " << try_ConnectUSBCount << std::endl;
  }
  while(m_viper.usb_connect() != 0 && try_ConnectUSBCount < 50);

  if(try_ConnectUSBCount == 49)
  {
    std::cout << "Error connecting to Viper over USB" << std::endl;
    // m_startContinuousReady_promise.set_value(false);
    return -1;
  }
  std::thread read_usb_thread = thread(&viper_ui::read_usb_data,this,&m_viper);	// start up the read thread
  read_usb_thread.detach();
  std::cout << "read_usb_thread ID: " << read_usb_thread.get_id();

  //IF Viper already started
  int rv = 0;
  int retrycount = 0;
  do
  {
    rv = print_single(&m_viper,is_continuous);
    if(rv != 0)
    {
      cout<<"Error Preparing for starting continuous ("<<rv<<")\n";
      this_thread::sleep_for(std::chrono::milliseconds(20));
      retrycount++;
      std::cout <<"retry print single: "<< retrycount;
    }
  }
  while(rv != 0 && retrycount < 50);

  retrycount = 0;
  do{
    // rv = start_continuous_acclerationMode(&m_viper);
    rv = start_continuous(&m_viper);
    if (rv!=0)
    {      
      cout<<"Error starting continuous ("<<rv<<")\n";
      this_thread::sleep_for(std::chrono::milliseconds(200));
      retrycount++;
      std::cout <<"retry starting continuous: "<< retrycount;
    }
  }while(rv != 0 && retrycount < 50);
  if(rv == 0)
  {
    // m_startContinuousReady_promise.set_value(true);
    LOG(INFO)<<"Alreay set value to True in Read Viper Continuous";
  }
  return rv;
}

std::future<bool> viper_ui::getReadyFuture()
{
  return m_startContinuousReady_promise.get_future();
}


std::array<std::array<double,viperDataNumPerSensor>,2> viper_ui::returnMyViperData()
{
  return m_sensorData.load();
}

float FractToFloat(int16_t fract, float factor)
{
    return static_cast<float>(fract) / 32768.0f/factor;
}
