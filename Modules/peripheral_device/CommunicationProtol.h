#ifndef _COMMUNICATION_PROTOL_H_
#define _COMMUNICATION_PROTOL_H_
/*
 *IP和端口分配
 *升降柱：60，8080
 *左手柄：120，9080
...
#define PREAMBLE 0x5857574B

#define MAMCARGSCOUNT 200

typedef enum eDeviceType
{
    DEV_HOST = 0x11,
    DEV_LIFTING=0x22,
    DEV_LEFT_HANDLE=0x33,
...
typedef enum eDevSta
{
    Dev_Sta_OK = 0x00,
    Dev_Sta_LEFTHANDLE_ERROR=0x1B,
    Dev_Sta_RIGHTHANDLE_ERROR=0x2B,
    Dev_Sta_LIFTING_ERROR=0x3B,
    Dev_Sta_LIGHT_ERROR=0x4B,
    Dev_Sta_VIPER_TRANSMITTER_ERROR=0x5B,
    Dev_Sta_UNKNOW_ERROR
} eDevSta;
...
typedef enum eCmds
{
    ...
    CMD_CTRL_WRITE=0xF2,
    CMD_READ_DATA=0xF3,
    CMD_UPLOAD_DATA=0xF4,
    CMD_RES_CMD=0xF5,
    CMD_READ_WRITE = 0xF6
} eCmds;

typedef enum eLiftingActions
{
    LIFTING_ACTIONS_SET_COLOR=0x1C,
    LIFTING_ACTIONS_SET_MODEL=0x2C,
    LIFTING_ACTIONS_SET_LIFT=0X3c,
    LIFTING_ACTIONS_SET_FOOTSUPPORT=0X4c
}eLiftingActions;

typedef enum eHandleActions
{
    HANDLE_ACTIONS_GET_SINGLE_DATA=0x4C,
    HANDLE_ACTIONS_SET_COLOR=0x5C,
    HANDLE_ACTIONS_SET_MODEL=0x6C,
}eHandleActions;

typedef enum eViper_Transmitter_Actions
{
    VIPER_TRANSMITTER_GET_SINGLE_DATA=0x7C,
...
typedef enum eLightColor
{
    ...
    LIGHT_COLOR_BLUE=0xC2,
    LIGHT_COLOR_GREEN=0xC3,
    LIGHT_COLOR_WHITE=0xC4,
    LIGHT_COLOR_YELLOW=0xC5
}eLightColor;
...
typedef enum eLiftingMotion
{
    LIFTING_MOTION_POWER_ON=0xE1,
    LIFTING_MOTION_POWER_OFF=0xE1,
    LIFTING_MOTION_UP=0xE3,
    LIFTING_MOTION_DOWN=0xE4,
    LIFTING_MOTION_BRAKE=0xE5
}eLiftMotion;

#pragma pack(push)
#pragma pack(1)
typedef struct _DATA_PAY_LOAD
{
    uint8_t size;//1
    eCmds type;//1
    uint8_t argscounter;//1
    uint8_t args[MAMCARGSCOUNT];
}DATA_PAY_LOAD;




typedef struct _COMMU_FRAME
{
    uint32_t preamble;//4
    uint32_t ID;//4
    uint8_t size;//1
    eDeviceType sender;//1
    eDeviceType recever;//1
    DATA_PAY_LOAD payload;
    uint32_t crc;//4
    uint16_t LF;//2
}COMMU_FRAME;
#pragma pack(pop)
...
//{
//	union u                          //!< 32-bit union of serial port settings
//	{
