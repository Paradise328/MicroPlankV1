...
int viper_ui::viper_Start_Directly()
{

  if (m_viper112.usb_connect()!=0){
    std::cout<<"\n\nError connecting to Viper over USB\n\n";
    return 1;
  }

  std::thread read_usb_thread = thread(&viper_ui::read_usb_data,this,&m_viper112);	// start up the read thread
  read_usb_thread.detach();

  int rv = 0;
  do{
    rv = start_continuous(&m_viper112);
    if (rv!=0)
...
//    cout<<"Error starting continuous ("<<rv<<")\n";
  ...
  int rvsc=0;
//  rv=start_continuous(&viper112);
  do{
    rvsc=start_continuous(&viper);
    cout<<"rvsc="<<rvsc<<"\n";
    if (rvsc!=0)
      cout<<"Error starting continuous ("<<rvsc<<")\n";
...
//  std::ofstream outfile("DelayTest.txt",std::ios::app);

  while (keep_reading_usb){

//      cout<<"resp_size="<<resp_size<<endl;
...
//  std::ofstream outfile("Position Accuracy Test Viper.txt",std::ios::trunc);
...
//          PosDataFromViper PosDataFromViperTmp;

//          PosDataFromViperTmp.viperDataL.viperData_X = senframeDatatemp.pno.pos[0];
//          PosDataFromViperTmp.viperDataL.viperData_Y = senframeDatatemp.pno.pos[1];
//          PosDataFromViperTmp.viperDataL.viperData_Z = senframeDatatemp.pno.pos[2];
//          PosDataFromViperTmp.viperDataL.viperData_Arzimuth = senframeDatatemp.pno.ori[0];
//          PosDataFromViperTmp.viperDataL.viperData_Elevation = senframeDatatemp.pno.ori[1];
//          PosDataFromViperTmp.viperDataL.viperData_Roll = senframeDatatemp.pno.ori[2];

//          senframeDatatemp=*(pfd+1);
//          PosDataFromViperTmp.viperDataR.viperData_X = senframeDatatemp.pno.pos[0];
//          PosDataFromViperTmp.viperDataR.viperData_Y = senframeDatatemp.pno.pos[1];
//          PosDataFromViperTmp.viperDataR.viperData_Z = senframeDatatemp.pno.pos[2];
//          PosDataFromViperTmp.viperDataR.viperData_Arzimuth = senframeDatatemp.pno.ori[0];
//          PosDataFromViperTmp.viperDataR.viperData_Elevation = senframeDatatemp.pno.ori[1];
//          PosDataFromViperTmp.viperDataR.viperData_Roll = senframeDatatemp.pno.ori[2];


          ViperSensorData viperDattemp,viperDattemp2;
          SENFRAMEDATA senframeDatatemp=*(pfd);
          viperDattemp.sensor1_pos[0]=senframeDatatemp.pno.pos[0];
          viperDattemp.sensor1_pos[1]=senframeDatatemp.pno.pos[1];
          viperDattemp.sensor1_pos[2]=senframeDatatemp.pno.pos[2];
          viperDattemp.sensor1_pos[3]=senframeDatatemp.pno.ori[0];
          viperDattemp.sensor1_pos[4]=senframeDatatemp.pno.ori[1];
          viperDattemp.sensor1_pos[5]=senframeDatatemp.pno.ori[2];

          senframeDatatemp=*(pfd+1);
          viperDattemp.sensor2_pos[0]=senframeDatatemp.pno.pos[0];
          viperDattemp.sensor2_pos[1]=senframeDatatemp.pno.pos[1];
          viperDattemp.sensor2_pos[2]=senframeDatatemp.pno.pos[2];
          viperDattemp.sensor2_pos[3]=senframeDatatemp.pno.ori[0];
          viperDattemp.sensor2_pos[4]=senframeDatatemp.pno.ori[1];
          viperDattemp.sensor2_pos[5]=senframeDatatemp.pno.ori[2];

//          senframeDatatemp=*(pfd+2);
//          viperDattemp2.sensor1_pos[0]=senframeDatatemp.pno.pos[0];
//          viperDattemp2.sensor1_pos[1]=senframeDatatemp.pno.pos[1];
//          viperDattemp2.sensor1_pos[2]=senframeDatatemp.pno.pos[2];
//          viperDattemp2.sensor1_pos[3]=senframeDatatemp.pno.ori[0];
//          viperDattemp2.sensor1_pos[4]=senframeDatatemp.pno.ori[1];
//          viperDattemp2.sensor1_pos[5]=senframeDatatemp.pno.ori[2];


//          outfile<<viperDattemp.sensor1_pos[0]<<" "<<viperDattemp.sensor1_pos[1]<<" "<<viperDattemp.sensor1_pos[2]<<" "
//                 <<viperDattemp2.sensor1_pos[0]<<" "<<viperDattemp2.sensor1_pos[1]<<" "<<viperDattemp2.sensor1_pos[2]<<"\n";
          m_ViperData.store(viperDattemp);
...
//          cout<<"\n";
 ...
 cout<<"fuck loop out 11"<<std::endl;
...
ViperSensorData  viper_ui::GetSeneorsData()
{
    return m_ViperData.load();
}
