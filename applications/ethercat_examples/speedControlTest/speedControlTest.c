...
unsigned char abSendData[120];
unsigned char abRecvData[120];
...
void parseCmd(char cmd[], char val[]){
  if(strcmp(val,"?")==0){
    ...
    //display actual position
        ...
        printf("hex position actual: %02X %02X %02X %02X\n",abRecvData[9],abRecvData[8],abRecvData[7],abRecvData[6]);
        int position = 0;
        if(abRecvData[15]>127){
          position=-(((255-abRecvData[9])*256+(255-abRecvData[8]))*256+(255-abRecvData[7]))*256+255-abRecvData[6];
        }else{
          position=((abRecvData[9]*256+abRecvData[8])*256+abRecvData[7])*256+abRecvData[6];
        }
        printf("dec position actual: %d\n",position);
    }
    ...
    //display actual velocity
        ...
        int velocity = 0;
        if(abRecvData[15]>127){
          velocity=-(((255-abRecvData[15])*256+(255-abRecvData[14]))*256+(255-abRecvData[13]))*256+255-abRecvData[12];
        }else{
          velocity=((abRecvData[15]*256+abRecvData[14])*256+abRecvData[13])*256+abRecvData[12];
        }
        printf(" decimal velocity actual: %d\n",velocity);
...
int init_joint(){
  int control_mode = 3,target_acceleration=15000,target_deceleration=15000,max_velocity=5000,control_word=128;

  printf("set control mode to %d\n",control_mode);
  abSendData[16]=control_mode%256;
  ...
  abSendData[0]=control_word%(256);
  abSendData[1]=control_word/256%256;
...
int update_speed(int target_velocity){
...
int main(){
    ...
    while(1){
        ...
        if(strcmp(cmd,"init")==0){
          printf("INITIALIZE the JOINT!!!\n");
          init_joint();
        }
        if(strcmp(cmd,"disable")==0){
          printf("DISABLE the JOINT!!!\n");
          disable_joint();
        }else if(strcmp(cmd,"enable")==0){
          printf("ENABLE the JOINT!!!\n");
          enable_joint();
        }else if(strcmp(cmd,"speed")==0){
          int speed = atoi(val);
          printf("SET the JOINT SPEED to %d\n",speed);
          update_speed(speed);
        }else if(strcmp(cmd,"stop")==0){
          printf("STOP the JOINT!!!\n");
          update_speed(0);
        }else{
          parseCmd(cmd,val);
        }
